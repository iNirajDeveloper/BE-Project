    ...
    if (Firebase.Firestore.getDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str())) {
      ...
      if (nameData.success) {
        ...
        if (timeClient.update()) {
          ...
          sendToFirestore(name, formattedDate, formattedTime);
        } else {
          Serial.println("Failed to update NTP time.");
        }
      } else {
...
void sendToFirestore(String name, String date, String time) {
  // Create a new document in the "Attendance" collection
  String documentPath = "Attendance/" + String(millis()); // Use millis() as a unique ID
  ...
  if (Firebase.Firestore.createDocument(&fbdo, FIREBASE_PROJECT_ID, "", documentPath.c_str(), content.raw())) {
    ...
    Serial.println("Failed to create document in Firestore.");
    Serial.println(fbdo.errorReason());
  }
}